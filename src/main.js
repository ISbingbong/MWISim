...
function populateAbilityDropdown() {
    fetch("./src/combatsimulator/data/abilityDetailMap.json")
        ...
        .then(data => {
            for (let i = 10; i <= 17; i++) {
                const abilityDropdown = document.getElementById(`selectAbility_${i}`);
                if (!abilityDropdown) {
...
function fillAbilities() {
    // Find the select elements and set their selected options
    let selectAbility1 = document.getElementById('selectAbility_1');
    let selectAbility2 = document.getElementById('selectAbility_2');
    let selectAbility3 = document.getElementById('selectAbility_3');
    let selectAbility4 = document.getElementById('selectAbility_4');
    
    // Find the correct option and set it as selected
    for (let i = 0; i < selectAbility1.options.length; i++) {
        if (selectAbility1.options[i].text === abilitiesList1[currentIndex1]) {
            selectAbility1.options[i].selected = true;
            break;
        }
    }
    
    for (let i = 0; i < selectAbility2.options.length; i++) {
        if (selectAbility2.options[i].text === abilitiesList2[currentIndex2]) {
            selectAbility2.options[i].selected = true;
            break;
        }
    }
    
    for (let i = 0; i < selectAbility3.options.length; i++) {
        if (selectAbility3.options[i].text === abilitiesList3[currentIndex3]) {
            selectAbility3.options[i].selected = true;
            break;
        }
    }
    
    for (let i = 0; i < selectAbility4.options.length; i++) {
        if (selectAbility4.options[i].text === abilitiesList4[currentIndex4]) {
            selectAbility4.options[i].selected = true;
            break;
        }
    }
    ...
    currentIndex3 = (currentIndex3 + 1) % abilitiesList3.length;
    currentIndex4 = (currentIndex4 + 1) % abilitiesList4.length;
}
...
function permuteStart() {
    ...
    // Generate all combinations of length 4
    const allPermutationsLevel = permuteLevel(abilityLevels, 4);

    // Populate the lists with combinations
    allPermutationsLevel.forEach(perm => {
        abilitiesLevelList1.push(perm[0]);
        abilitiesLevelList2.push(perm[1]);
